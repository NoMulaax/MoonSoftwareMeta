...
export async function POST(req) {
    ...
    if (!username || username.length > 50 || username.length < 2) {
        return new Response(JSON.stringify({
            error: {
                code: 400,
                message: "Please enter a valid username for this client less than 30 characters and above 1 character."
            }
        }), { status: 400 });
    }
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return new Response(JSON.stringify({
            error: { code: 401, message: "Authentication required. Please enter an API key" }
        }), { status: 401 });
    }
    ...
    if (apiUserError) {
        ...
        return new Response(JSON.stringify({
            error: { code: 500, message: "A server error occurred! Please try again later or contact support." }
        }), { status: 500 });
    }
    ...
    if (apiUser.api_uses_left < 1) {
        return new Response(JSON.stringify({
            user: null, error: { code: 403, message: "You have no API uses left." }
        }), { status: 403 });
    }
